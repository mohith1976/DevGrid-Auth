...
export default function DashboardOwnerApplications({ user }: { user?: any }){
  ...
  const load = async ()=>{
    ...
    try{
      ...
      let res;
      if (token) {
        res = await axios.get('https://api.digitaldevgrid.tech/api/proposals/owner', { headers: { Authorization: `Bearer ${token}` } });
      } else if (user && user.id) {
        // dev fallback: allow fetching by userId query when token not available
        res = await axios.get(`https://api.digitaldevgrid.tech/api/proposals/owner?userId=${encodeURIComponent(user.id)}`);
      } else {
                            ...
                            <>
                              <button
                                ...
                                onClick={async () => {
                                  const token = localStorage.getItem('token');
                                  try {
                                    const res = await axios.post(`https://api.digitaldevgrid.tech/api/proposals/${p._id}/accept`, { applicantId: a.userId }, { headers: { Authorization: `Bearer ${token}` } });
                                    if (res?.data && res.data.success === false) {
                                    ...
                                    if (!teamId) {
                                      try {
                                        const r = await axios.get(`https://api.digitaldevgrid.tech/api/proposals/${p._id}`, { headers: { Authorization: `Bearer ${token}` } });
          ...
          <div className="modal-card">
            ...
            <div style={{ display:'flex', justifyContent:'flex-end', gap:8, marginTop:12 }}>
              ...
              <button className="btn" onClick={async ()=>{
                try {
                  const token = localStorage.getItem('token');
                  await axios.post(`https://api.digitaldevgrid.tech/api/proposals/${confirmReject.proposalId}/reject`, { applicantId: confirmReject.applicantId }, { headers: { Authorization: `Bearer ${token}` } });
                } catch (e:any) {
                  // ignore
                }
