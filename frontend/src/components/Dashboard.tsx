...
export default function Dashboard({ user }: { user: User }){
  ...
  const refresh = async () => {
    ...
    try {
      const [pRes, statsRes, reposRes, teamsRes] = await Promise.all([
        axios.get('https://api.digitaldevgrid.tech/api/projects', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('https://api.digitaldevgrid.tech/api/projects/profile/me', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('https://api.digitaldevgrid.tech/api/projects/repos', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('https://api.digitaldevgrid.tech/api/teams', { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const loadAggregate = async () => {
    ...
    if (!token) return;
    try {
      const res = await axios.get('https://api.digitaldevgrid.tech/api/projects/profile/aggregate', { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="dashboard-shell reveal in">
      ...
      <ConfirmModal open={!!confirmModal?.open} title="Publish repository" body="Include collaborators?" items={confirmModal?.repo?.contributors || []} onCancel={() => setConfirmModal(null)} onConfirm={async (include:boolean) => {
        try {
          ...
          const token = localStorage.getItem('token');
          const res = await axios.post('https://api.digitaldevgrid.tech/api/projects/publish',{ repo: repoUrl, includeCollaborators: include }, { headers: { Authorization: `Bearer ${token}` } });
          if (res.data?.success) {
