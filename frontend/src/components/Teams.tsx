...
export default function Teams({ user }: { user?: any }){
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = token ? await axios.get('https://api.digitaldevgrid.tech/api/teams', { headers: { Authorization: `Bearer ${token}` } }) : await axios.get('https://api.digitaldevgrid.tech/api/teams');
