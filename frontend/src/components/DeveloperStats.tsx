...
export default function DeveloperStats(){
    ...
    const fetchAll = async () => {
      ...
      try{
        const [aggRes, reposRes] = await Promise.all([
          axios.get('http://localhost:3000/api/projects/profile/aggregate', { headers: { Authorization: `Bearer ${token}` } }),
  ...
  useEffect(()=>{
    ...
    sse.onmessage = (evt) => { try { const p = JSON.parse(evt.data||'{}'); if (p?.source) { axios.get('http://localhost:3000/api/projects/profile/aggregate', { headers: { Authorization: `Bearer ${token}` } }).then(r=>setAggregate(r.data?.aggregate || null)); } } catch(e){} };
